 ...
 *   toast.warn("Sem conexão com o banco.");
 *   toast.info("Salvando...");
 *
 *   <ToastContainer /> — coloque uma vez no layout
 */
...
export interface ToastItem {
  ...
  title: string;
  message?: string;
  duration?: number; // ms — 0 = não fecha automaticamente
}
...
function Toast({ item, onClose }: { item: ToastItem; onClose: (id: string) => void }) {
  ...
  return (
    <div
      className={`flex items-start gap-3 rounded-xl border ${cfg.border} ${cfg.bg} px-4 py-3 shadow-xl backdrop-blur-sm w-full max-w-sm animate-in slide-in-from-right-4 duration-300`}
      role="alert"
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {item.message && (
          <p className="text-xs text-white/50 mt-0.5 leading-relaxed">{item.message}</p>
        )}
      </div>
      <button
...
// ─── INLINE ALERT (para erros persistentes em formulários) ────
...
export function Alert({
  ...
  const Icon = cfg.icon;
  return (
    <div className={`flex items-start gap-3 rounded-xl border ${cfg.border} ${cfg.bg} px-4 py-3`} role="alert">
      ...
      {onClose && (
        <button onClick={onClose} className="shrink-0 text-white/20 hover:text-white/60 transition-colors">
          <X size={13} />
        </button>
      )}
    </div>
