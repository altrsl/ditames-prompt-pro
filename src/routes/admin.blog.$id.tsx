    ...
    async function load() {
      try {
        ...
        if (!isNew) {
          ...
          if (error || !data) {
            showError("Artigo não encontrado", "Verifique se o registro existe e tente novamente.");
            return;
          }
          setTitle(data.title);
          setBody(data.body);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await writeAuditLog({
        ...
        module: "blog",
        new_value: { title, published },
      });

      toast.success(
        isNew ? "Artigo criado com sucesso!" : "Artigo atualizado!",
        published ? "Publicado e visível no blog." : "Salvo como rascunho."
      );
    ...
    <div className="p-6 md:p-8 max-w-3xl mx-auto">
      ...
      <div className="space-y-5">
        ...
        <div className="flex gap-3 pt-2">
          ...
          <Link to="/admin/blog" className="inline-flex items-center gap-2 rounded-lg border border-white/10 px-6 py-3 text-sm font-semibold text-white/60 hover:text-white hover:border-white/30 transition-colors">
            Cancelar
          </Link>
        </div>
      </div>
