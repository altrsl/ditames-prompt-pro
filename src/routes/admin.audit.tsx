...
function AdminAudit() {
  ...
  async function load(currentLimit = limit) {
    try {
      ...
      const data = await getAuditLogs(moduleFilter || undefined, undefined, currentLimit);
      setLogs(data);
    } catch (e) {
  ...
  if (userLoaded && !hasPermission(user, "view_audit_log")) {
    return (
      <div className="p-8 text-center text-white/30 text-sm">
        Você não tem permissão para visualizar o histórico de alterações.
      </div>
    );
  }
  ...
  return (
    <div className="p-6 md:p-8 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          onClick={() => { setLimit(PAGE_SIZE); load(PAGE_SIZE); }}
